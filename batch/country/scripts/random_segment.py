...
# save results
#rand_segment_data = pd.DataFrame(results, columns = ['sample','cluster','cluster_size', 'cost', 'silhouette_score','total_points', 'explained_variance', 'diameter_miles', 'containment_health', 'avg_infections'])
...
rand_segment_data.to_csv('batch/country/data/random_segment_clusters_' + str(job_id) + '.csv')