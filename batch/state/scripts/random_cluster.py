...
rand_cluster_data.to_csv('batch/state/data/unimodal_rc_analysis.csv')
...
rand_cluster_data.to_csv('batch/state/data/sir_rc_analysis.csv')
...
for i in range(samples):
    random_clusterings = rand_cliquer.sample(1)
    #rc_data = wc.auxiliary_info(rand_cliquer, wpool, [G_miles, G_density], health_index = containment_health, infection_data = data, extra_info = [i])
...
# save results
#rand_cluster_data = pd.DataFrame(rand_cluster_data, columns = ['clustering','cluster','cluster_size', 'cost','silhouette_score',
# 'total_points','explained_variance', 'diameter_miles', 'diameter_density', 'containment_health', 'avg_infections'])
rand_cluster_data = pd.DataFrame(rand_cluster_data, columns = ['clustering', 'cluster','cluster_size', 'cost', 'total_points','explained_variance',
            'silhouette', 'geo_silhouette','health_silhouette','geo_pairwise', 'health_pairwise', 'containment_health', 'avg_infections'])

rand_cluster_data.to_csv('batch/state/data/wav_rc_analysis.csv')