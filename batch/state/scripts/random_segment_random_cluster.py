...
# save results                
#rand_segment_rand_cluster_data = pd.DataFrame(random_results, columns = ['sample','cluster','cluster_size', 'cost', 'silhouette_score','total_points','explained_variance', 'diameter_miles', 'diameter_density', 'containment_health', 'avg_infections'])
...
rand_segment_rand_cluster_data.to_csv('batch/state/data/random_segment_random_clusters_' + str(job_id) + '.csv')