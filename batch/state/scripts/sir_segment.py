...
if total_jobs > 1:
    est_waves.to_csv("batch/state/data/sir_waves" + str(job_id) + ".csv")
    est_cuts.to_csv("batch/state/data/sir_cuts" + str(job_id) + ".csv")
else:
    est_waves.to_csv("batch/state/data/sir_waves.csv")
    est_cuts.to_csv("batch/state/data/sir_cuts.csv")


#elbows = pd.DataFrame(elbows, columns = locations)
#elbows.to_csv('batch/state/data/sir_elbows.csv')
